...
--	ADDRESS2 
	...
	ci.moli_cdc 												AS cdc,	
	CASE WHEN ci.DOCSTATUS = 'RE' THEN
		CASE WHEN CI.GRANDTOTAL >= 0 THEN 
			'Cancelar'
		ELSE
			'Inutilizar'
		END
	...
	CASE WHEN ci.DOCSTATUS = 'RE' THEN
		CASE WHEN CI.GRANDTOTAL >= 0 THEN 
			'Cancelado por el Integrador'
		ELSE
			'Inutilizado por el Integrador'
		END
	ELSE 
		NULL
	END															AS evento_motivo,
	...
	cil.QTYINVOICED 												AS item_cantidad,
	CASE WHEN (mp.isTAXINCLUDED = 'Y')  THEN 
		cil.PRICELIST
	ELSE 
		ROUND(((cil.LINENETAMT + cil.taxamt)/cil.QTYINVOICED), 8) 
...
--	ci.MOLI_creditnotereason									AS nota_credito_motivo
	...
	-- Datos de Documento Asociado
	--
	CASE WHEN cd_asociado.name = 'AR Factura Electrónica' THEN 
		1
	ELSE 
		2
	END 													AS doc_aso_formato,
	
	CASE WHEN cd_asociado.name = 'AR Nota Crédito Electrónica' THEN 
		invoice_asociado.moli_cdc
	ELSE 
		NULL 
	END  														AS doc_aso_cdc,
	...
	CASE WHEN cd.name = 'AR Nota Crédito Electrónica' THEN 
		NULL
	ELSE 
		 NULL--invoice_asociado.timbrado
	END															AS doc_aso_timbrado,
	
	CASE WHEN cd.name = 'AR Nota Crédito Electrónica' THEN 
		NULL
	ELSE 
		SUBSTR(invoice_asociado.DOCUMENTNO, 10,3) 
	END 														AS doc_aso_establecimiento,
	
	CASE WHEN cd.name = 'AR Nota Crédito Electrónica' THEN 
		NULL
	ELSE 
		SUBSTR(invoice_asociado.DOCUMENTNO, 14,3) 
	END 														AS doc_aso_punto,
	
	CASE WHEN cd.name = 'AR Nota Crédito Electrónica' THEN 
		NULL
	ELSE 
		SUBSTR(invoice_asociado.DOCUMENTNO, 18,7) 
	END  														AS doc_aso_numero,
	
	CASE WHEN cd.name = 'AR Nota Crédito Electrónica' THEN 
		NULL
	ELSE 
		invoice_asociado.DATEINVOICED 
	...
	FROM 
		C_Invoice ci
		INNER JOIN C_ORDER rma ON
			(ci.C_ORDER_ID = rma.C_ORDER_ID)
		INNER JOIN C_ORDER order_asociado ON
			(rma.ORIG_ORDER_ID = order_asociado.C_ORDER_ID)
		INNER JOIN C_INVOICE invoice_asociado ON
			(ORDER_ASOCIADO.C_ORDER_ID = invoice_asociado.C_ORDER_ID)
		INNER JOIN C_InvoiceLine cil ON
			(ci.C_Invoice_ID = cil.C_Invoice_ID)
		LEFT OUTER JOIN M_Product mp ON
			(cil.M_Product_ID = mp.M_Product_ID)
		INNER JOIN C_Currency cc2 ON
			(ci.C_Currency_ID = cc2.C_Currency_ID)
		INNER JOIN C_DocType cd ON
			(ci.C_DocType_ID = cd.C_DocType_ID AND cd.name = 'AR Nota Crédito Electrónica')
		INNER JOIN C_DocType cd_asociado ON
			(invoice_asociado.C_DocType_ID = cd_asociado.C_DocType_ID)
		INNER JOIN C_PaymentTerm pt ON
			(ci.C_PaymentTerm_ID = pt.C_PaymentTerm_ID)
		INNER JOIN C_BPartner cb ON
			(ci.C_BPartner_ID = cb.C_BPartner_ID)
...
SELECT *  FROM C_INVOICE ci WHERE C_INVOICE_ID IN ( 1959244)
SELECT 
		(ci.LINENETAMT + ci.TAXAMT ) ,
		(ci.LINENETAMT + ci.TAXAMT ) / ci.QTYINVOICED ,
		round(((ci.LINENETAMT + ci.TAXAMT ) / ci.QTYINVOICED), 6),
		round(((ci.LINENETAMT + ci.TAXAMT ) / ci.QTYINVOICED), 8)*ci.QTYINVOICED AS linenetAmount,
		round(((ci.LINENETAMT + ci.TAXAMT ) / 4), 8)*4 AS linenetAmount2,

ci.LINENETAMT, ci.LINETOTALAMT, ci.TAXAMT, ci.QTYENTERED , ci.QTYINVOICED, ci.PRICELIST 
FROM C_INVOICELINE ci WHERE C_INVOICE_ID =  1959251
...
SELECT transaccion_id, tipo_documento, cdc, estado, evento_motivo 
FROM RV_C_Invoice_FacturaSend 
WHERE 1=1 
...
GROUP BY transaccion_id, tipo_documento, cdc, estado, evento_motivo, establecimiento, punto, numero 
ORDER BY establecimiento, punto, numero 
